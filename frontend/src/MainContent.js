...
function MainContent({ session }) {
  ...
  const handleGenerateFlashcards = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/generate-flashcards/${documentId}`, {
        headers: { 'Authorization': `Bearer ${session.access_token}` },
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
  ...
  const handleGenerateMockTest = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/generate-mock-test/${documentId}?count=${questionCount}`, {
        headers: { 'Authorization': `Bearer ${session.access_token}` }
      });
      const data = await res.json();
